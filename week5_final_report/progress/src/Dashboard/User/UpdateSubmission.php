...
// Get registration_id from GET parameter
$registration_id = isset($_GET['id']) ? intval($_GET['id']) : null;
...
if ($result->num_rows > 0) {
    ...
    $preselected_event_id = $existing_submission['Event_ID'];
} else {
    header("Location: RecordSubmissions.php");
    exit();
}
...
$stmt->bind_param("i", $preselected_event_id);
$stmt->execute();
$result = $stmt->get_result();
if ($result->num_rows > 0) {
...
<html>
    ...
    <main>
        ...
        <div class="container">
            <h2 style="font-size: 32px; margin: 20px; text-align: center;">Update Submission for <?php echo htmlspecialchars($event['Nama_event']); ?></h2>

            <div class="temp">
            <form action="UpdateSubmit.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="registration_id" value="<?php echo $registration_id; ?>">
                <input type="hidden" name="event_id" value="<?php echo $preselected_event_id; ?>">

                <label for="Judul">Judul (optional):</label>
                <input type="text" name="Judul" value="<?php echo htmlspecialchars($existing_submission['Judul'] ?? ''); ?>" placeholder="Enter Judul" class="action2" required>

                <label for="submission_pdf">PDF (optional):</label>
                <input type="file" name="submission_pdf" accept=".pdf" class="action2">

                <div class="parent">
                        <button id="btn" type="button" onclick="location.href='RecordSubmissions.php'">Back to Submissions</button>
                        <button id="btn" type="submit" name="update_item" class="action2">Update Submission</button>
                </div>
            </form>
        </div>
        </div>
