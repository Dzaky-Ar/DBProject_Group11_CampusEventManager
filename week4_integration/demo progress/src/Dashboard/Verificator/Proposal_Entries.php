...
if ($_POST) {
    if (isset($_POST['approve_proposal'])) {
        $verificator->approveProposal($_POST['proposalID'], $verificatorID);
    } elseif (isset($_POST['reject_proposal'])) {
        $verificator->rejectProposal($_POST['proposalID'], $verificatorID);
    }
}

$verificatorID = $verificator->getVerificatorIDByEmail($_SESSION['Email']);
$pending_proposals = $verificator->getPendingProposals($verificatorID);

// Check if a proposal ID is provided to show details
$selected_proposal = null;
if (isset($_GET['proposal_id'])) {
    $selected_proposal = $verificator->getProposalByID($_GET['proposal_id'])->fetch_assoc();
}
?>
...
<body>
    ...
    <main>
        <h1 id="special">Pending Proposals</h1>
        ...
        <table>
            ...
            if ($pending_proposals) {
                while ($row = $pending_proposals->fetch_assoc()): ?>
                <tr>
                    <td><a href="?proposal_id=<?= $row['Proposal_ID'] ?>" style="color: #007bff; text-decoration: none;"><?= $row['Proposal_ID'] ?></a></td>
                ...
                echo "<tr><td colspan='12'>No pending proposals</td></tr>";
            }
            ?>
        </table>
    </main>
